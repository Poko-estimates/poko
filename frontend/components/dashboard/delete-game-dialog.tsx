...
function DeleteGameDialog({
  ...
  function handleDelete() {
    startTransition(async () => {
      ...
      toast.add({ type: "success", title: `Deleted “${game.name}”` })

      // Drop the ?game= param when the open game is the one that just went,
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <div className="flex flex-col gap-1.5">
          ...
          <AlertDialogDescription>
            <span className="font-medium text-primary">{game.name}</span> and
